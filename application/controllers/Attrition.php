...
class Attrition extends CORE_Controller
{

    function __construct() {
        parent::__construct('');
        $this->validate_session();
        if($this->session->userdata('right_attrition_view') == 0 || $this->session->userdata('right_attrition_view') == null) {
            redirect('../Dashboard');
        }
    ...
    public function index() {
        ...
        $data['position'] = $this->RefPosition_model->get_list('is_deleted = 0');
        $data['title'] = 'Attrition Report';

        $this->load->view('attrition_view', $data);
    }
