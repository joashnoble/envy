                            ...
                            <div class="panel panel-default">
                                ...
                                <div class="panel-body table-responsive" style="padding-top:8px;">
                                    <div class="row" style="padding: 10px;">
                                            ...
                                            <div class="col-md-3">
                                                <label  style="float:left;margin-top:5px;margin-left: 20px;font-weight: bold;" for="inputEmail1">Department Filter :</label>

                                                <select class="form-control" name="department_filter_list" id="department_filter_list" data-error-msg="Pay Period Filter is required" required>
                                                <option value="all">All</option>
                                                <?php
                                            ...
                                            <div class="col-sm-6">
                                                <button type="button" class="btn col-sm-12 form-control" id="print_personnel_list" style="background-color:#27ae60; color:white;width: 100px;margin-top: 30px;"><i class="fa fa-print"></i> PRINT</button>
                                                <button type="button" class="btn col-sm-12 btn-primary form-control" id="export_personnel_list" style="color:white;margin-left: 20px;margin-top: 30px;width: auto;"><i class="fa fa-file-excel-o"></i> EXPORT EXCEL</button>
                                                
                                                <button type="button" class="btn col-sm-12 btn-info form-control" id="email_personnel_list" style="color:white;margin-left: 20px;margin-top: 30px; width: 100px;"><i class="fa fa-envelope"></i> EMAIL</button>
                                            </div>
                                        </div>
                                    </div>
                                    <hr>
...
$(document).ready(function(){
    ...
    //$('#department_filter_list').val(d.getFullYear()).trigger("change")
    ...
    $.ajax({
        "dataType":"html",
        "type":"POST",
        "url":"Hris_Reports/reports/personnel-list/"+filter_department+"/"+filter_branch+"/0",
        beforeSend : function(){
    ...
    $('#print_personnel_list').click(function(event){
            showinitializeprint();
    ...
    $('#export_personnel_list').on('click', function() {
        filter_department = $('#department_filter_list').val();
        filter_branch = $('#branch_filter_list').val();
        window.open("Hris_Reports/reports/export-personnel-list/"+filter_department+"/"+filter_branch+"/0","_self");
    });

    $('#email_personnel_list').on('click', function() {
        filter_department = $('#department_filter_list').val();
        filter_branch = $('#branch_filter_list').val();
    ...
    $.ajax({
        "dataType":"json",
        "type":"POST",
        "url":"Hris_Reports/reports/email-personnel-list/"+filter_department+"/"+filter_branch+"/0",
